...
FormLead = (function() {
  ...
  FormLead.prototype._repopulateDealers = function(excludeDealerId, selectSibling) {
    var $selectSibling, dealer, i, len, ref, p, rbId;
    $selectSibling = $(selectSibling);
    $selectSibling.attr('disabled', true).empty().append($('<option/>'));
    ref = this._dealers;
    rbId = ["650","652","4550","4518","4576","630","627","4602","4514S","4514"];
    for (i = 0, len = ref.length; i < len; i++) {
      dealer = ref[i];
      for (p = 0; p < rbId.length; p++) {
        if (dealer.idSalesforce == rbId[p]) {
  ...
  FormLead.prototype._populateVehicleModels = function() {
    ...
    return $.ajax({
      ...
      success: (function(_this) {
        return function(data, textStatus, jqXHR) {
          var i, len, results1, vehicleModel, p, rbId;
          _loading.remove();
          _this._vehicles = data;
          results1 = [];
          rbId = ["Q3333","A3333","A4444","Q7777"];
          for (i = 0, len = data.length; i < len; i++) {
            vehicleModel = data[i];
            for (p = 0; p < rbId.length; p++) {
              if(vehicleModel.idSalesforce == rbId[p]){
  ...
  FormLead.prototype._populateDealers = function() {
    ...
    $.ajax({
      ...
      success: (function(_this) {
        return function(data, textStatus, jqXHR) {
          var dealer, i, len, results1, p, rbId, rbLen;
          _loading.remove();
          _this._dealers = data;
          results1 = [];
          rbId = ["650","652","4550","4518","4576","630","627","4602","4514S","4514"];
