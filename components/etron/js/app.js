          ...
          "fields": [
            {
              ...
              "attr": {
                "type": "text",
                "name": "birthday",
                "data-parsley-pattern": "[0-9]{2}\/[0-9]{0,2}\/[0-9]{4}",
                "data-parsley-group": "birthdayGroup"
              }
            }, {
              ...
              "title": "ESCOLHA UM MODELO ADICIONAL (OPCIONAL)",
              "placeholderTag": "",
              "tag": "select",
              "attr": {
                "name": "vehiclemodel",
                "class": "vehiclemodel-optional"
              }
            }
          ]
        }
...
FormLead = (function() {
  ...
  FormLead.prototype._buildFields = function() {
    ...
    this._radioInterest.push(new RadioCustom({
      ...
      callback: (function(_this) {
        return function() {
          ...
          $('.cpf').eq(0).find('select').attr({
            "data-parsley-required": "true"
          });
          $('.birthday').eq(0).find('select').attr({
            "data-parsley-required": "true"
          });
    ...
    this._radioInterest.push(new RadioCustom({
      ...
      callback: (function(_this) {
        return function() {
          ...
          $('.seller, .vehiclemodel, .cpf, .birthday').hide();
          $('.seller').find('select').val('');
          $('.vehiclemodel').find('select').val('');
          $('.cpf').find('input').val('');
          $('.birthday').find('input').val('');
          $('.vehiclemodel-default').parent().css('display', 'block');
    ...
    this._checkBox = new CheckBoxCustom({
      text: 'Quero receber novidades da Audi Brasil.',
    ...
    vehicleModel1 = $('.vehiclemodel select')[0];
    vehicleModel2 = $('.vehiclemodel select')[1];
    ...
    $(vehicleModel1).on('change', (function(_this) {
      return function() {
        var excludeVehicleId;
        excludeVehicleId = $(vehicleModel1).val();
        return _this._repopulateVehicles(excludeVehicleId, vehicleModel2);
      };
    })(this));
  ...
  FormLead.prototype.disableFields = function() {
    ...
    if ((this.config != null) && (this.config.type === 'newsletter' || this.config.type === 'etron')) {
  ...
  FormLead.prototype._repopulateDealers = function(excludeDealerId, selectSibling) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      dealer = ref[i];
      if (dealer.id !== excludeDealerId) {
        $selectSibling.append($('<option />').attr('value', dealer.id).text(dealer.initial + " - " + dealer.name));
      }
    }
    ...
    return $.ajax({
      ...
      success: (function(_this) {
        return function(data, textStatus, jqXHR) {
          ...
          for (i = 0, len = data.length; i < len; i++) {
            state = data[i];
            _formSelect.append($('<option/>').text(state.name).attr({
              'value': state.id
            }));
          }
    ...
    return $.ajax({
      ...
      success: (function(_this) {
        return function(data, textStatus, jqXHR) {
          ...
          for (i = 0, len = data.length; i < len; i++) {
            vehicleModel = data[i];
            results1.push(_formSelect.append($('<option/>').text(vehicleModel.name).attr('value', vehicleModel.id)));
          }
          return results1;
        };
      })(this)
  ...
  FormLead.prototype._onSubmit = function(e) {
    ...
    if (this._form.parsley().isValid() && this._flagGo && this._isValidPersonalFields()) {
      ...
      $.ajax({
        url: (this.config.type === 'newsletter' || this.config.type === 'etron') ? window.URL_NEWSLETTER_SAVE : window.URL_LEAD_SAVE,
        type: 'post',
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify(this._prepareData(dealers, vehicles)),
        error: function(jqXHR, textStatus, errorThrown) {},
        success: (function(_this) {
          return function(data, textStatus, jqXHR) {
  ...
  FormLead.prototype._prepareData = function(dealers, vehicles) {
    ...
    _data = {};
    if (this.config.type === 'newsletter' || this.config.type === 'etron') {
      _data = {
