          ...
          "fields": [
            {
              ...
              "title": "DATA DE AGENDAMENTO (Informe até duas datas de sua preferência. Entraremos em contato com você para confirmação.)",
              "incorrectText": "*preencha o campo data de agendamento corretamente",
              "background": "",
              "tag": "textarea",
              "attr": {
                "type": "textarea",
                ...
                "data-parsley-group": "chassiGroup"
              }
            }
          ]
        }
...
FormLead = (function() {
  function FormLead() {
    ...
    this._populateYearsSelect = bind(this._populateYearsSelect, this);
    // there is no solicitation field anymore
    // this._populateSolicitation = bind(this._populateSolicitation, this);
    this.config = {};
  }
  ...
  FormLead.prototype.create = function(p_data, region_data) {
    ...
    this._populateYearsSelect();
    // this._populateSolicitation();

    $('input[name=cpf]').attr({
      "data-parsley-required": true
    });

    $('select[name=seller], select[name=vehiclemodel]').attr ({
      "data-parsley-required": true
    });
  };
  ...
  FormLead.prototype._buildFields = function() {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      if(field.attr.name === "description")
        this._formElementTag = field.tag != null ? "<" + field.tag + "/>" : '<textarea/>';
    ...
    this._formGroupInterest = this.element.find('.formGroup.interest');
    this._formGroupInterest.hide();
    ...
    $('.seller, .vehiclemodel, .birthday').show();
    ...
    this._radioInterest.push(new RadioCustom({
      ...
      callback: (function(_this) {
        return function() {
          ...
          $('.vehiclemodel-default').parent().find('.labelTitle').text('ESCOLHA UM MODELO*');
...
this._formGroupNews = $('<div/>').addClass("formGroup newsletter");
  ...
  FormLead.prototype._populateMaps = function() {
    return $.ajax({
      url: window.URL_DEALER_LIST_JSON3,
      dataType: 'json',
      error: (function(_this) {
        return function(jqXHR, textStatus, errorThrown) {
          return console.log("AJAX Error: " + textStatus);
  ...
  FormLead.prototype._populateYearsSelect = function() {
    var _formSelect = $('select[name=yearModel], select[name=yearFabrication]');
    ...
    for(var i = 1970; i <= 2018; i++) {
      results1.push(_formSelect.append($('<option/>').text(i)));
    }
  }

  /*
  FormLead.prototype._populateSolicitation = function () {
    var _formSelect = $('select[name=solicitation]');
    _formSelect.append($('<option />'));

    var selectOptions = [
      'Agendamento de revisão / serviço',
      'Consulta de recall',
      'Assistência 24 horas',
      'Acessórios geunínos Audi',
      'Pedido de manual',
      'Dúvidas sobre o funcionamento do veículo',
    ...
    for (var i = 0; i < selectOptions.length; i++) {
      results1.push(_formSelect.append($('<option/>').text(selectOptions[i])));
    }
  }
  */
  ...
  FormLead.prototype._populateDealers = function() {
    ...
    return this.element.find('.seller select');
  };
        ...
        success: (function(_this) {
          return function(data, textStatus, jqXHR) {
            if (data.status === "success") {
              _this._successMessageTitle.html('<strong>Cadastro enviado com sucesso. Em breve entraremos em contato para finalizar seu agendamento. Obrigado!</strong><br/>.');
            } else {
